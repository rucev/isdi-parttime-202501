...
function playRound() {
    guessInput = prompt('try your first guess number'); //primer intento de adivinar un numero
    if (guessInput === null) {  //en caso de que le de a cancelar, salirnos
        ...
        giveTurnFeedback() // le damos feedback de que tan lejos o cerca esta el numero que ha dicho
    }
...
/**
 * Función que setea las variables necesarias para cada ronda en su valor inicial
*/
function setGameRoundVariables() {
    goalNumber = Math.floor(Math.random() * 10) + 1; //setea el primer numero a adivinar
    counter = 1; //cuenta intentos por ronda
    errors = 0; //cuenta errores por ronda
}

/**
 * Da feedback al usuario dependiendo del número que haya escrito.
 * @returns null
 */
function giveTurnFeedback() {
...
/**
 * Da feedback al usuario dependiendo de si ha ganado la ronda o no.
*/
function giveRoundFeedback() {
    if (errors === 4) { //coge el numero de errores de la ronda que estabamos jugando y si es 4 hace lo siguiente:
        ...
        setGameRoundVariables() //reseteamos variables de la ronda
    }
    ...
    if (guessNumber === goalNumber) { //en caso de que haya acertado el numero
        ...
        if (roundsWon === 3) { //en caso de que se hayan ganado las tres rondas necesarias para terminar el juego
            ...
            giveWinGameFeedback()//damos feedback del juego terminado
...
/**
 * Da feedback de las rondas jugadas y los intentos hechos por ronda
*/
function giveWinGameFeedback() {
...
if (isGameOn) { //en caso de que quiera jugar --> ejecutamos el siguiente código
    ...
    setGameRoundVariables() //setea las variables para la ronda
