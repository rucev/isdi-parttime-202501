...
describe('getHomePosts', () => {
    ...
    it('GIVEN an existent user with posts published adn following users WHEN getHomePosts is called it populates the post info and filtrates it THEN returns a populated and formated posts array', () => {
    ...
    /*
        it('GIVEN an existent user id and no posts published WHEN getHomePosts is called THEN returns an empty array', () => {
            const userData = { username: 'Test-1', password: 'Test1!', email: 'email@mail.com', following: [], followers: [] }
            return data.users.create(userData)
                .then(user => {
                    return getHomePosts(user._id.toString())
                        .then(posts => {
                            expect(posts).to.be.an('array')
