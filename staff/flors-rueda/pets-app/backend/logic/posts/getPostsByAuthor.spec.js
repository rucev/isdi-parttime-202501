...
describe('getPostsByAuthor', () => {
    ...
    it('GIVEN an existent user with multiple posts self published WHEN getPostsByAuthor is called it populates the post info THEN returns a populated and formated posts array filtered by author', () => {
    ...
    it('GIVEN an existent user id and no posts published WHEN getPostsByAuthor is called THEN returns an empty array', () => {
        const user1Data = { username: 'Test-1', password: 'Test1!', email: 'email@mail.com', following: [], followers: [] }
        const user2Data = { username: 'Test-2', password: 'Test2!', email: 'email2@mail.com', following: [], followers: [], avatar: 'random.com/image.png' }
        const user3Data = { username: 'Test-3', password: 'Test3!', email: 'email3@mail.com', following: [], followers: [] }
        ...
        return data.users.create([user1Data, user2Data, user3Data])
            .then(([user1, user2, user3]) => {
                ...
                return data.posts.create([post1Data, post2Data])
                    .then((_) => {
                        return getPostsByAuthor(user1._id.toString(), user1._id.toString())
                            .then(posts => {
                                expect(posts).to.be.an('array')
    ...
    it('GIVEN a not existent user id WHEN getPostsByAuthor is called does not found the user THEN throws ExistenceError', () => {
        const fakeId = new data.ObjectId()
        return getPostsByAuthor(fakeId.toString(), fakeId.toString())
            .catch(error => {
    ...
    it('GIVEN a not existent author user id WHEN getPostsByAuthor is called does not found the author THEN throws ExistenceError', () => {
        const fakeId = new data.ObjectId()
        const user1Data = { username: 'Test-1', password: 'Test1!', email: 'email@mail.com', following: [], followers: [] }

        return data.users.create(user1Data)
            .then(user => {
                return getPostsByAuthor(user._id.toString(), fakeId.toString())
                    .catch(error => {
                        expect(error).to.be.instanceOf(errors.ExistenceError)
                        expect(error.message).to.be.a('string')
